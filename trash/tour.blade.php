        ...
        <div class="row">
            ...
            <div class="col-lg-7 mb-4">
                @if (!empty($tour['images']))
                    <img src="{{ $tour['images'][0] }}" class="img-fluid rounded mb-3 w-100"
                         alt="{{ $tour['title'] }}" style="max-height:400px; object-fit:cover;">
                    @if (count($tour['images']) > 1)
                        <div class="d-flex gap-2 flex-wrap">
                            @foreach (array_slice($tour['images'], 1) as $img)
            ...
            <div class="col-lg-5">
                <div class="card border-0 shadow-sm p-4 sticky-top" style="top:20px;">
                    <h4 class="fw-bold mb-1">{{ $tour['title'] }}</h4>
                    @if ($tour['location'])
                    ...
                    <form action="{{ route('rezgo.storefront.cart.add') }}" method="POST" id="rezgo-booking-form">
                        @csrf
                        <input type="hidden" name="uid" value="{{ $tour['uid'] }}">
                        <input type="hidden" name="title" value="{{ $tour['title'] }}">
                        <input type="hidden" name="image" value="{{ $tour['images'][0] ?? '' }}">
