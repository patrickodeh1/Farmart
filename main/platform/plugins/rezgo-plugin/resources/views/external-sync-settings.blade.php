...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-lg-8">
                <form method="POST" action="{{ route('rezgo.external-sync.save') }}" id="externalSyncForm">
                    @csrf
                        ...
                        <div class="card-body">
                            <div class="form-group mb-3">
                                ...
                                <input type="text" class="form-control @error('host') is-invalid @enderror" 
                                       id="host" name="host" value="{{ $host }}" placeholder="localhost or IP address">
                                @error('host')
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6 mb-3">
                                    ...
                                    <input type="number" class="form-control @error('port') is-invalid @enderror" 
                                           id="port" name="port" value="{{ $port }}" min="1" max="65535">
                                    @error('port')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="form-group col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control @error('database') is-invalid @enderror" 
                                           id="database" name="database" value="{{ $database }}" placeholder="your_custom_db">
                                    @error('database')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control @error('username') is-invalid @enderror" 
                                           id="username" name="username" value="{{ $username }}" placeholder="database_user">
                                    @error('username')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group col-md-6 mb-3">
                                    <label for="password" class="form-label">Password {{ !$host ? '<span class="text-danger">*</span>' : '' }}</label>
                                    <input type="password" class="form-control @error('password') is-invalid @enderror" 
                                           id="password" name="password" placeholder="{{ !$host ? 'Database password (required for first setup)' : 'Leave blank to keep existing password' }}" {{ !$host ? 'required' : '' }}>
                                    @error('password')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <small class="form-text text-muted">{{ !$host ? 'Required to establish initial connection' : 'Only enter a new password if you want to change it' }}</small>
                                </div>
                            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            testConnectionBtn.addEventListener('click', function() {
                ...
                fetch('{{ route("rezgo.external-sync.test-connection") }}', {
                    method: 'POST',
                    headers: {
            ...
            testTablesBtn.addEventListener('click', function() {
                ...
                fetch('{{ route("rezgo.external-sync.table-status") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
            ...
            createTablesBtn.addEventListener('click', function() {
                ...
                fetch('{{ route("rezgo.external-sync.create-tables") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
